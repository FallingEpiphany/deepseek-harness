...
flowchart LR
  ...
  svc_mcpConnections["ctx.mcpConnections<br/>Authenticated MCP connection directory"]
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.mcpConnections` | `core` | [`mcp-client`](../packages/mcp/mcp-client) | - | - | - | 配套插件在既有工具作用域内解析已认证的资源和提示，无需复制凭据。 |
